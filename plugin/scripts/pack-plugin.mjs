...
const ASSETS_EXCLUDES = [
    ...
    // page.php change stops needing one of them, move it back into this
    // list.
    'branding/brandguide.html',
    'branding/fonts/beasca.woff',
    // logo_aurora.svg and logo_aurora_tagline.svg (the static, non-animated
    // wordmarks) aren't used anywhere anymore — the plugin's Admin_Page
    // uses logo_aurora_animated_tagline.svg, and the showcase site's
    // index.html uses logo_aurora_animated.svg (also excluded below since
    // that one's only ever read by the site, never by the plugin itself).
