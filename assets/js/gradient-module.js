...
(function ($) {
    ...
    function findTextLeaves(textEl) {
        return textEl.querySelectorAll('.aurora-char, .aurora-word, .aurora-line, .aurora-line-wrap .aurora-word, .letter, .word');
    }
    ...
    function applyText(textEl, trackEl, data, id) {
        ...
        if (leaves.length === 0) {
            ...
            for (var lj = 0; lj < leaves.length; lj++) {
                ...
                if (parentW > 0 && parentH > 0 && leafRect.width > 0) {
                    leaf.style.backgroundSize     = parentW + 'px ' + parentH + 'px';
                    ...
                    leaf.style.backgroundSize     = '100% 100%';
                    leaf.style.backgroundPosition = '0% 0%';
                }
            }
        }
        ...
        if (leaves.length > 0) {
            selector = '.aurora-gradient-text[data-aurora-gradient-instance="' + id + '"], ' +
                       '.aurora-gradient-text[data-aurora-gradient-instance="' + id + '"] .aurora-char, ' +
                       '.aurora-gradient-text[data-aurora-gradient-instance="' + id + '"] .aurora-word, ' +
                       '.aurora-gradient-text[data-aurora-gradient-instance="' + id + '"] .aurora-line';
            for (var lk = 0; lk < leaves.length; lk++) {
                leaves[lk].style.backgroundSize = '300% 300%';
            }
        }
