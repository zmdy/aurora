 ...
 * every real (non-editor) page that uses Text Animation. Bundles all
 * effects so that every effect is immediately and synchronously available
 * in the registry without fragile per-effect chunk HTTP requests or
 * async race conditions during page load.
 */

// Every import below runs the effect file's own top-level
// registerEffect() call as a side effect.
